...
def inp():
    ...
    conn = mycon.cursor()
    conn.execute("INSERT INTO employee VALUES(%a,%a,%a,%a,%a,%a,%a,%a,%a)"%(e1,e2,e3,e4,e5,e6,e7,e8,e9))
    mycon.commit()
...
def emp_screen():
    ...
    l8 = tkinter.Label(rootE, text="EMAIL")
    l8.place(x=50, y=290)
    t6 = tkinter.Entry(rootE)
    t6.place(x=90, y=290)
    l9 = tkinter.Label(rootE, text="CONTACT NO")
    l9.place(x=50, y=260)
...
def delling():
    ...
    mycon = pymysql.connect(host="localhost", user="root", password="986064", database="HMS")
    conn = mycon.cursor()
    conn.execute("select * from employee where EMP_ID=%a"%de)
    p=conn.fetchone()
    if (p != None):
        conn.execute("DELETE from employee where EMP_ID=%a"%de)
