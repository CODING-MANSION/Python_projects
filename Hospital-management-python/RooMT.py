...
def room_button():
    ...
    conn = mycon.cursor()
    r1=P_id.get()
    r2=room_t.get(tkinter.ACTIVE)[:-8]
    r3=room_no.get(tkinter.ACTIVE)
    r4=rate.get()
    r5=da.get()
    r6=dd.get()
    conn.execute("INSERT INTO ROOM VALUES(%a,%a,%a,%a,%a,%a)"%(r1, r3, r2, r4, r5, r6))
...
def update_button():
    try:
        ...
        r6=dd.get()
        conn = mycon.cursor()
        conn.execute("Select * from ROOM where PATIENT_ID=%a"%(r1))
        p=conn.fetchone()
        if len(p) != 0:
            conn.execute("UPDATE ROOM SET ROOM_NO=%a,ROOM_TYPE=%a,RATE=%a,DATE_ADMITTED=%a,DATE_DISCHARGED=%a where PATIENT_ID=%a"%(r3, r2, r4, r5, r6,r1))
            tkinter.messagebox.showinfo("HMS DATABASE SYSTEM", "ROOM DETAILS UPDATED")
            mycon.commit()
        else:
            tkinter.messagebox.showinfo("MEDANTA DATABASE SYSTEM", "PATIENT IS NOT ALLOCATED A ROOM")
...
def ROOMD_button():
    ...
    c1 = mycon.cursor()
    r1=P_iid.get()
    c1.execute('select * from  ROOM  where PATIENT_ID=%a'%r1)
    p=c1.fetchone()
    if (p==None):
        tkinter.messagebox.showinfo("HMS DATABASE SYSTEM","PATIENT NOT ALLOCATED ROOM")
    else:
        c1.execute("SELECT NAME,ROOM_NO,ROOM_TYPE FROM ROOM NATURAL JOIN PATIENT where PATIENT_ID=%a;"%r1)
        ii=c1.fetchone()
...
def Room_all():
    ...
    L=['SINGLE ROOM: Rs4500','TWIN SHARING : Rs2500','TRIPLE SHARING: Rs2000']
