...
def date_up():
    ...
    b1 = P_id.get()
    b2 = dd.get()
    conn.execute("UPDATE ROOM SET DATE_DISCHARGED=%a where PATIENT_ID=%a"%(b2, b1))
    mycon.commit()
...
def up():
    ...
    b8 = price.get()
    conn.execute("INSERT INTO TREATMENT VALUES(%a,%a,%a,%a)"%(b1, b3, b4, b5,))
    conn.execute("INSERT INTO MEDICINE VALUES(%a,%a,%a,%a)"%(b1, b6, b7, b8,))
    mycon.commit()
    tkinter.messagebox.showinfo("HMS DATABASE SYSTEM", "BILLING DATA SAVED")

def calci():
    global b1
    mycon = pymysql.connect(host="localhost", user="root", password="986064", database="HMS")
    conn = mycon.cursor()
    conn.execute("Select sum(T_COST+ (M_COST*M_QTY) +(DATE_DISCHARGED-DATE_ADMITTED)*RATE) FROM ROOM NATURAL JOIN TREATMENT natural JOIN MEDICINE where PATIENT_ID=%a;"%b1)
    u=conn.fetchall()
    print(u)
    mycon.commit()
    for ii in u:
        ...
        uu=tkinter.Label(rootB,text=ii)
