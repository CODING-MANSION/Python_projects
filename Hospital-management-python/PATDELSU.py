...
def Search_button():
    ...
    c1=mycon.cursor()
    inp_s=entry.get()
    c1.execute('select * from PATIENT where PATIENT_ID=%a'%(inp_s))
    p = c1.fetchone()
    if p==None:
        errorS=tkinter.Label(rootS,text="PATIENT RECORD NOT FOUND")
        errorS.pack()
    else:
        c1.execute('SELECT * FROM PATIENT NATURAL JOIN CONTACT_NO where PATIENT_ID=%a;'%(inp_s))
        t=c1.fetchall()
        for i in t:
...
def Delete_button():
    ...
    conn= mycon.cursor()
    inp_d = entry1.get()
    conn.execute("select * from PATIENT where PATIENT_ID=%a"%(inp_d))
    p=conn.fetchone()
    if p==None:
        errorD = tkinter.Label(rootD, text="PATIENT RECORD NOT FOUND")
        errorD.pack()
    else:
        conn.execute("delete from appointment where PATiENT_ID=%a;" % inp_d)
        conn.execute("delete from ROOM where PATiENT_ID=%a;" % inp_d)
        conn.execute("delete from CONTACT_NO where PATiENT_ID=%a;"%inp_d)
        conn.execute("DELETE FROM PATIENT where PATIENT_ID=%a;"%(inp_d))
...
def up1():
    ...
    u9 = pat_CT.get()
    u10 = pat_address.get()
    conn.execute("Select * from PATIENT where PATIENT_ID=%a"%(u1))
    p=conn.fetchall()
    print(p)
    if len(p) != 0:
        conn.execute('UPDATE PATIENT SET NAME=%a,SEX=%a,DOB=%a,BLOOD_GROUP=%a,ADDRESS=%a,CONSULT_TEAM=%a,EMAIL=%a where PATIENT_ID=%a'%( u2, u3, u4, u5, u10, u9, u8,u1))
        conn.execute('UPDATE CONTACT_NO set CONTACTNO=%a,ALT_CONTACT=%a WHERE PATIENT_ID=%a'%( u6, u7,u1,))
        tkinter.messagebox.showinfo("MEDANTA DATABSE SYSTEM", "DETAILS UPDATED INTO DATABASE")
        mycon.commit()

    else:
        tkinter.messagebox.showinfo("HMS DATABSE SYSTEM", "PATIENT IS NOT REGISTERED")
