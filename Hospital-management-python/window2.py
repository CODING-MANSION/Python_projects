...
def ex():
    ...
    root1.destroy()
def menu():
    global root1,button1,button2,button3,button4,button5,m,button6,rootp
    if rootp != None:
         rootp.destroy()
    root1=tkinter.Tk()
...
def PAT():
    ...
    global rootp,regform,id,name,dob,sex,email,ct,addr,c1,c2,bg,SUBMIT,menubar,filemenu,back,SEARCH,DELETE,UPDATE,root1
    root1.destroy()
    rootp=tkinter.Tk()
    rootp.title("HMS PATIENT FORM")
...
#input patient form
def IN_PAT():
    global pp1, pp2, pp3, pp4, pp5, pp6, pp7, pp8, pp9, pp10,ce1,conn,root1
    mycon = pymysql.connect(host="localhost", user="root", password="986064", database="HMS")
    conn = mycon.cursor()
    pp1=pat_ID.get()
    pp2=pat_name.get()
    ...
    try:
        conn.execute('INSERT INTO PATIENT VALUES(%a,%a,%a,%a,%a,%a,%a,%a)'%(pp1,pp2,pp3,pp4,pp5,pp8,pp9,pp10))
        conn.execute('INSERT INTO CONTACT_NO VALUES (%a,%a,%a)'%(pp1,pp6,pp7))
        tkinter.messagebox.showinfo("HMS DATABASE SYSTEM","DETAILS INSERTED INTO DATABASE")
